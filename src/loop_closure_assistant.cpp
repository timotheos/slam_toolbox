...
/*****************************************************************************/
{
  ...
  ssClear_manual_ = node_->create_service<slam_toolbox::srv::Clear>
      ("clear_changes", std::bind(&LoopClosureAssistant::clearChangesCallback, 
      this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
  
  ssLoopClosure_ = node_->create_service<slam_toolbox::srv::LoopClosure>
      ("manual_loop_closure", std::bind(&LoopClosureAssistant::manualLoopClosureCallback,
      this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
  ...
  ssInteractive_ = node_->create_service<slam_toolbox::srv::ToggleInteractive>
      ("toggle_interactive_mode", std::bind(&LoopClosureAssistant::interactiveModeCallback,
      this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));

  node_->declare_parameter("interactive_mode", interactive_mode_);
  ...
  node_->set_parameter(rclcpp::Parameter("enable_interactive_mode", enable_interactive_mode_));
  ...
  for (ConstGraphIterator it = graph->begin(); it != graph->end(); ++it) {
    ...
    if (interactive_mode && enable_interactive_mode_)
    {
      visualization_msgs::msg::InteractiveMarker int_marker =
        vis_utils::toInteractiveMarker(m, node_, 0.3);
...
/*****************************************************************************/
{
  ...
  {
    RCLCPP_WARN(node_->get_logger(), "Called manual loop closure"
...
/*****************************************************************************/
{
  ...
  node_->declare_parameter("paused_processing", interactive_mode);
  return true;
}
...
/*****************************************************************************/
{
  if(!enable_interactive_mode_)
  {
    RCLCPP_WARN(node_->get_logger(),"Called Clear changes with interactive mode disabled. Ignoring.");
    return false;
  }
